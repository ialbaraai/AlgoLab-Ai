...
LinearRegressionApp::LinearRegressionApp() {}

void LinearRegressionApp::run()
{
    ...
    SDL_Window* window = SDL_CreateWindow("AlgoLab AI | Linear Regression", 1050, 625, SDL_WINDOW_RESIZABLE);
    ...
    SDL_Surface* icon = IMG_Load("lr_icon.png");
    ...
    while (is_running)
    {
        ...
        if (ImGui::BeginTabBar("##maintabs"))
        {
            ...
            if (ImGui::BeginTabItem("Settings"))
            {
                ...
                ImGui::Spacing();
                ImGui::TextColored(COL_MUTED, "Max Epochs");
        ...
        {
            m_LR = LinearRegression_T(0.001, maxIter);

            for (auto& p : points) {
                p.Position.X /= 1920.0f;
                p.Position.Y /= 1080.0f;
            }

            m_LR.Simulate(points);
            ...
            std::cout << "y = " << m_LR.mbpair().first << " * x + " << m_LR.mbpair().second << '\n';

            for (Point_T& point : unknownPoints)
            {
                double normalizedX = point.Position.X / 1920.0;
                double normalizedY = m_LR.predict(normalizedX);
                point.Position.Y = (float)(normalizedY * 1080.0);
                point.Cluster = 0;
                point.Color = { 55, 255, 55, 255 };
                point.rerect();

                std::cout << point.Name << " -> predicted Y: " << point.Position.Y << '\n';
            }
        }
        ImGui::End();
        ...
        if (adddata)
        {
            ...
            if (ImGui::BeginTabBar("##addtabs"))
            {
                if (ImGui::BeginTabItem("Point"))
                {
                    ...
                    {
                        Point_T p(std::string(nameBuf), x, y, { 55, 255, 55, 255 });
                ...
                if (ImGui::BeginTabItem("Unknown Point"))
                {
                    ...
                    bool canAdd = nameValid && xValid;
                    if (!canAdd) ImGui::BeginDisabled();

                    ImGui::PushStyleColor(ImGuiCol_Button, COL_ACCENT);
                    if (ImGui::Button("Add Unknown Point", ImVec2(btn2, 0)))
                    {
                        Point_T p(std::string(nameBuf), x, 0.0f, { 255, 155, 55,255 });
                        p.Cluster = -1;
            ...
            if (csvr.isread())
            {
                ...
                if (!csvfilepath.empty() && !headers.empty())
                {
                    ...
                    if (ImGui::Button("Generate Points", ImVec2(140, 0)))
                    {
                        ...
                        {
                            Point_T p(curdata.m_Row[selectedLabelIdx], curdata.convert<float>(selectedXIdx).value(), curdata.convert<float>(selectedYIdx).value(), { 255, 255, 255, 255 });
                            p.Cluster = 0;

                            if (curdata.convert<int>(selectedClusterIdx).value() == -1)
                            {
                                p.Color = { 255, 155, 55, 255 };
                                unknownPoints.push_back(p);
                            }
                            else
                            {
                                p.Color = { 55, 255, 55, 255 };
                                points.push_back(p);
                            }
                        }
        ...
        for (auto& p : points) p.render(renderer, renderingFrameX, renderingFrameY);
        for (auto& up : unknownPoints) up.render(renderer, renderingFrameX, renderingFrameY);
